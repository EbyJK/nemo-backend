...
def clean_notification_email(body: str):
    ...
    text = re.sub(r"\b(Open|View|Click here|Download)\b", "", body, flags=re.IGNORECASE)
    text = re.sub(r"http\S+", "", text)
    text = re.sub(r"\s+", " ", text)
...
def classified_emails():
    ...
    for m in messages:
        ...
        if any(k in subject.lower() for k in business_keywords):
            is_corporate = True
        ...
        # 3️⃣ Summary
        clean_body = clean_notification_email(body)
        summary_text, summary_conf = generate_summary(subject, clean_body)
        ...
        # 4️⃣ Tasks
        tasks_output = extract_tasks_backend(
            EmailInput(subject=subject, body=body, sender=sender, summary=summary_text)
        )
        ...
        output.append({
            ...
            "category": label.lower(),
            "confidence": confidence,
            "detailed_category": detect_category(email_text),
            "has_attachment": has_attachment,
            "attachments": attachments,
            "attachment_count": len(attachments)
        })
